...
export function SandboxedIframe({ id, scriptSrc }) {
    return (_jsx("iframe", { id: id, className: 'sandboxed-iframe', 
        // @ts-expect-error
        csp: [
            "default-src 'self'",
            ...
            '',
        ].join('; '), height: 0, sandbox: 'allow-scripts', srcDoc: buildSandboxedWidget(scriptSrc), title: 'code-container', width: 0, style: { border: 'none' } }));
}
